...
class LSTMTagger(nn.Module):
    """
    A Bi-LSTM model for sequence tagging without CRF/Viterbi decoding.
    We simply output logits for each token, and train/test with cross-entropy.
    """
    def __init__(self, feature_dim, hidden_dim, num_tags, num_layers=2,
        ...
        # We use cross-entropy with ignore_index set to pad_idx
        self.loss_fn = nn.CrossEntropyLoss(ignore_index=pad_idx)

    def forward(self, features, lengths, tags=None):
        """
        If tags is provided, returns cross-entropy loss.
        Otherwise returns argmax predictions.
        """
        ...
        if tags is not None:
            ...
            # Flatten the batch+sequence into a single dimension for CE
            loss = self.loss_fn(logits.view(-1, self.num_tags), tags.view(-1))
            return loss
        else:
            # For inference, return predicted tag indices via argmax
...
def getSentencesAndTags(file_path):
    """
    Reads a file in CoNLL-like format (word/tag pairs, blank line separates sentences)
    and returns a list of (words, tags) tuples.
    """
...
def prepare_data(train_path, val_path, params):
    """
    Uses GloVe embeddings only; no CountVectorizer. 
    """
...
def generate_submission(model, test_path, feature_processor, word_vocab, tag_vocab,
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        for words, _ in tqdm(test_data):
            ...
            with torch.no_grad():
                # Forward pass without tags => get predictions via argmax
                predicted_indices = model(features_tensor, lengths, tags=None)
                predicted_indices = predicted_indices.squeeze(0).cpu().numpy()

            predicted_tags = [idx_to_tag[i] for i in predicted_indices]

            # Write word and predicted tag
